\grey{Можно использовать и калибровку отдельно для каждого AOD, не задумываясь тогда про SVD. В рамках этой работы такой подход не тестировался. Хочется подчеркнуть, что подход с SVD гарантировано работает и для double AOD [ссылка], где crosstalk между двумя направлениями может быть сильнее.}
